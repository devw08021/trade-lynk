...
import {
  useTradePaidMutation,
  useTradeCancelMutation,
  useTradeDisputeMutation,
  useTradeCompleteMutation,
} from "@/services/p2pService";

const TRADE_DURATION_MINUTES = 15;
const PAGE_SIZE = 15;

export default function TradeChatWithLoaderAndTimer({
  params,
}: {
  ...
  const [statusButtonLoader, setStatusButtonLoader] = useState(false);
  ...
  //status update
  const [tradePaid] = useTradePaidMutation();
  const handlePaidButton = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      setStatusButtonLoader(true);
      const { success, result, message } = await tradePaid({
        id: slug,
      }).unwrap();
      if (message) {
      ...
      if (err && (err as any).data?.message) {
  ...
  const [tradeComplete] = useTradeCompleteMutation();
  const handleCompleteButton = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      setStatusButtonLoader(true);
      const { success, result, message } = await tradeComplete({
        id: slug,
      }).unwrap();
      if (message) {
        toastSuccess(`${message}`);
  ...
  const [tradeCancel] = useTradeCancelMutation();
  const handleCancelButton = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      setStatusButtonLoader(true);
      const { success, result, message } = await tradeCancel({
        id: slug,
      }).unwrap();
      if (message) {
        toastSuccess(`${message}`);
  ...
  const [tradeDispute] = useTradeDisputeMutation();
  const handleDisputeButton = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      setStatusButtonLoader(true);
      const { success, result, message } = await tradeDispute({
        id: slug,
      }).unwrap();
      if (message) {
        toastSuccess(`${message}`);
        ...
        <Card className="flex flex-col h-full overflow-hidden min-h-0">
          ...
          <CardContent className="flex flex-col gap-4">
            ...
            <div className="flex justify-between">
              ...
              <span>
                {order?.receiveValue} {order?.firstCoin}
              </span>
            </div>
            <div className="flex justify-between">
