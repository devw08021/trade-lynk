...
export default function TradeHistoryTable({ side, crypto, fiat, paymentMethod }) {

    const [offersList, setOffersList] = useState<any>([]);
    const [offersCount, setOffersCount] = useState(0)
    const [page, setPage] = useState(0)

    const { data: offers, isLoading: isLoadingOffers, isError: isOffersError } = useGetMyTradesQuery({
        side: side,
        crypto: crypto,
        fiat: fiat,
                                                        ...
                                                        : offer.status === 3 ? 'secondary'
                                                            : offer.status === 4 ? 'secondary'
                                                                : offer.status === 5 ? 'destructive'
                                                                    : offer.status === 6 ? 'destructive'
                                                                        : 'destructive'
                                        }>
                                            {["Open", "Paid", "Completed", "Cancelled", "Dispute", "Dispute resolved", "Timed Out"][offer.status]}
                                        </Badge>
                                    </div>
                                </td>
